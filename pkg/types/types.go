...
type ConsumerHeartbeat struct {
	...
	SubscribedTopics   []byte    `gorm:"type:json;not null"`                                                               // 消费者订阅的Topic列表，JSON格式，如["topic-A", "topic-B"]
	AssignedPartitions []byte    `gorm:"type:json;not null"`                                                               // 协调器分配给消费者的分区，JSON格式，如{"topic-A": [0, 2]}
