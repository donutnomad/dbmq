...
func (d *MqRepo) GetConsumerHeartbeat(ctx context.Context, groupID, consumerID string) (*db.ConsumerHeartbeat, error) {
...
// 这是消费者心跳循环使用的主要函数
func (d *MqRepo) UpsertConsumerHeartbeat(ctx context.Context, groupID, consumerID string, subscribedTopics []string) error {
...
func (d *MqRepo) MarkConsumerOffline(ctx context.Context, groupID, consumerID string) error {
	now := time.Now()
...
// 保留此方法以支持老的删除逻辑（如果需要）
func (d *MqRepo) DeleteConsumerHeartbeat(ctx context.Context, groupID, consumerID string) error {
	return d.db.WithContext(ctx).
		Where("`group_id` = ?", groupID).
		Where("`consumer_id` = ?", consumerID).
...
func (d *MqRepo) FindActiveConsumers(ctx context.Context, groupID string, timeout time.Duration) ([]db.ConsumerHeartbeat, error) {
...
func (d *MqRepo) FindAllConsumers(ctx context.Context, groupID string, timeout time.Duration) ([]db.ConsumerHeartbeat, error) {
...
func (d *MqRepo) GetConsumerGroupGeneration(ctx context.Context, groupID string) (*db.ConsumerGroupGeneration, error) {
...
func (d *MqRepo) CommitOffset(ctx context.Context, groupID string, generationID uint, p db.PartitionInfo, lastConsumedMessageID int64) error {
...
func (d *MqRepo) BatchCommitLastConsumeMessageID(ctx context.Context, groupID string, generationID uint, consumedIds map[db.PartitionInfo]int64) error {
