...
type Repo interface {
	// CreateBatch 批量创建消息
	CreateBatch(ctx context.Context, messages []*db.Message) error
	// Fetch 获取单分区消息
	...
	GetLatestIDs(ctx context.Context, partitions []db.PartitionInfo) (map[db.PartitionInfo]int64, error)
